...
pub struct MainOutputs {
    pub split_joints_command: MainOutput<JointsCommand<f32>>,
}

impl Split {
    pub fn new(_context: CreationContext) -> Result<Self> {
        Ok(Self {
            interpolator: MotionFile::from_path("etc/motions/split.json")?.try_into()?,
        })
    }
    ...
    pub fn cycle(&mut self, context: CycleContext) -> Result<MainOutputs> {
        ...
        if motion_selection.current_motion == MotionType::Split {
            self.interpolator
